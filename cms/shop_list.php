...
if (isset($_GET['selected1'])) {
    ...
    $G_selected1 = $_SESSION['selected_shopC'] = $row_RecshopC['c_id'];
}

$query_Recshop = "SELECT data_set.*, class_set.c_title as c_title FROM data_set LEFT JOIN class_set ON data_set.d_class2 =class_set.c_id WHERE d_class1 = 'shop' AND d_class2='" . $G_selected1 . "' ORDER BY d_sort ASC, d_date DESC";
...
//$_SESSION['selected_shop']=$G_selected2;
//echo $query_Recshop;
...
if ($G_changeSort == 1 || $G_delchangeSort == 1) {
    ...
    if ($G_changeSort == 1) {
        if (isset($_GET['now_d_id'])) {
            header("Location:shop_list.php?selected1=" . $G_selected1 . "&pageNum=" . $_GET['pageNum'] . "&totalRows=" . $_GET['totalRows'] . "#" . $_GET['now_d_id']);
        } else {
            header("Location:shop_list.php?selected1=" . $G_selected1 . "&pageNum=" . $_GET['pageNum'] . "&totalRows=" . $_GET['totalRows']);
        }
    } else if ($G_delchangeSort == 1) {
        header("Location:shop_list.php?selected1=" . $G_selected1 . "&pageNum=" . $_GET['pageNum']);
    }
}
                            ...
                            <form action="shop_process.php" method="post" name="form1" id="form1">
                                ...
                                <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1">
                                    ...
                                    <tr <?php if ($i%2==0): ?>bgcolor='#E4E4E4'<?php endif ?>>
                                        ...
                                        <td align="center" class="table_data">
                                            <select name="d_sort" id="d_sort" onchange="changeSort('<?php echo $pageNum; ?>','<?php echo $totalRows; ?>','<?php echo $row_Recshop['d_id']; ?>',this.options[this.selectedIndex].value,<?= $G_selected1 ?>)">
...
<script type="text/javascript">
	...
	function changeSort(pageNum, totalRows, now_d_id, change_num, selected1) {
	    window.location.href = "shop_list.php?selected1=" + selected1 + "&changeSort=1" + "&now_d_id=" + now_d_id + "&change_num=" + change_num + "&pageNum=" + pageNum + "&totalRows=" + totalRows;
	}
