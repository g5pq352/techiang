...
function file_process(PDO $pdo, $file_name, $deal_type) {
    ...
    for ($j = 0; $j < count($_FILES['upfile']['name']); $j++) {
        ...
        if ($tmp_name != '') //如果有上傳檔案
        {
            ...
            $file_item = explode(".", $_FILES['upfile']['name'][$j]);
            $file_type = end($file_item); //將檔案已"."分開，放到陣,列呼叫array最後一個資料,為檔案副檔名


            if ($file_type == "jpeg" || $file_type == "jpg" || $file_type == "gif" || $file_type == "bmp" || $file_type == "png" || $file_type == "tif") {
