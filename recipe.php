...
// 拿來計算全部有幾則
$workTotal = $DB->query("SELECT * FROM data_set, file_set, class_set WHERE d_class1='recipe' AND d_class2=c_id AND (d_class2=? || $ryder_cat=0) AND d_id=file_d_id AND file_type='recipeCover' AND d_active=1 ORDER BY ".$sort, [$ryder_cat]);
$pageTotalCount = count($workTotal);
$totalpage = ceil($pageTotalCount / $maxItem);

//使用
$work = $DB->query("SELECT * FROM data_set, file_set, class_set WHERE d_class1='recipe' AND d_class2=c_id AND (d_class2=? || $ryder_cat=0) AND d_id=file_d_id AND file_type='recipeCover' AND d_active=1 ORDER BY ".$sort." LIMIT ?, ?", [$ryder_cat, $limit, $maxItem]);
	...
	<div class="newsWrap">
		...
		<ul class="catList grid-x">
			...
			<?php foreach($cat as $c) : ?>
				<li class="<?php if(isset($ryder_cat) && $ryder_cat == $c['c_id']) : ?>current<?php endif ?>"><a href="recipe.php?c=<?=$c['c_id'] ?>" data-sub="1">
